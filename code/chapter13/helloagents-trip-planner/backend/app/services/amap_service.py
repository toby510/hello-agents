...
class AmapService:
    ...
    def search_poi(self, keywords: str, city: str, citylimit: bool = True) -> List[POIInfo]:
        ...
        try:
            ...
            # 这里简化处理,实际应该解析JSON
            print(f"POI搜索结果: {result[:2000]}...")  # 打印前200字符

            # ======================
            # 🔥 安全解析：兼容空值、非JSON、JSON格式
            # ======================
            import json
            poi_list = []

            # ======================
            # 🔥 核心修复：提取纯JSON部分（去掉前面的文字）
            # ======================
            json_start = result.find('{')  # 找到第一个 {
            pure_json = result[json_start:]  # 从 { 开始截取后面所有内容
            ...
            for poi in pois:
                try:
                    # 经纬度（你的返回里没有经纬度，给默认值避免报错）
                    longitude = 120.153576  # 杭州默认经度
                    latitude = 30.287459  # 杭州默认纬度

                    # 构建POIInfo对象
                    poi_info = POIInfo(
                        id=poi.get("id", ""),
                        name=poi.get("name", ""),
                        type=poi.get("typecode", ""),  # 用 typecode 作为类型
                        address=poi.get("address", ""),
                        location=Location(
                            longitude=longitude,
                            latitude=latitude
                        ),
                        tel=poi.get("tel")  # 没有则为None
                    )
                    poi_list.append(poi_info)
                except Exception as e:
                    print(f"⚠️  单条POI解析失败: {e}")
                    continue

            print(f"✅ 成功解析 {len(poi_list)} 个POI信息")
            return poi_list
